...
(function ($, document) {
  ...
  var option = {
    init: function () {
      ...
      $(
        "#mcb-table-bar tbody, #mcb-table-icons tbody, #mcb-table-toggle tbody"
      ).initSettings();
    ...
    onReady: function () {
      ...
      $("#mcb-table-contacts thead").on(
        {
          mouseenter: function () {
            $(this)
              .removeClass("wp-ui-text-highlight")
              .addClass("wp-ui-highlight");
          },
          mouseleave: function () {
            $(this)
              .removeClass("wp-ui-highlight")
              .addClass("wp-ui-text-highlight");
          },
        },
        ".mcb-action"
      );
      ...
      option.tbody.on("click", ".mcb-row-toggle-parameters", function (event) {
        ...
        var button = $(this).children("i"),
          ...
          parameters = option.tbody.find(
            '.mcb-parameter[data-contact-id="' + contactId + '"]'
          );
        ...
        if ("true" == button.attr("aria-expanded")) {
          parameters
            .first()
            .find(".mcb-parameter-value input, .mcb-parameter-value textarea")
            .focus();
        } else {
      ...
      option.tbody.on("click", ".mcb-row-pick-icon", function (event) {
        ...
        $("body")
          ...
          .on("click", "#mcb-icon-picker-container div a", function (event) {
            ...
            if ("back" === $(this).attr("data-direction")) {
              list
                .find("li:gt(" + (list.children().length - 26) + ")")
                .prependTo(list);
            } else {
        ...
        $("body")
          ...
          .on(
            "input",
            "#mcb-icon-picker-container div input",
            function (event) {
              event.preventDefault();
              event.stopPropagation();
      ...
      // Add contact
      $(
        "#mcb-integration-icons .mcb-action, #mcb-integration-buttons .mcb-action"
        ...
        var contactType = $(this).attr("data-contact-type"),
          contactId =
            option.tbody.children(".mcb-contact").maxId("contact") + 1,
